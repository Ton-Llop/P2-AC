    ...
    unsigned int xor)         /* history xor address flag */
{
  ...
  case BPredAlloy:
  {
    ...
    if (!d->config.alloy.pabht)
      fatal("no se pudo reservar memoria para PaBHT");
    ...
    {
      fatal("no se pudo reservar memoria para las PHT");
    }

    // Inicializar contadores (1 = weakly not taken)
                 ...
                 md_addr_t baddr)              /* branch address */
{
  ...
  case BPredAlloy:
  {
    ...
    int votos_tomado = 0;
    if (pred_dir->config.alloy.tabla_pht_global[idx_global] >= 2)
      votos_tomado++;
    if (pred_dir->config.alloy.tabla_pht_local[idx_local] >= 2)
      votos_tomado++;
    if (pred_dir->config.alloy.tabla_pht_combi[idx_mixed] >= 2)
      votos_tomado++;
                                                     ...
                                                     * used on mispredict recovery */
{
  ...
  case BPredAlloy:
    if ((MD_OP_FLAGS(op) & (F_CTRL|F_UNCOND)) != (F_CTRL|F_UNCOND)) {
      ...
      dir_update_ptr->pdir1 = (char *)dir_update_ptr->alloy_combi;
